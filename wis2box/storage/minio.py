...
class MinioStorage(StorageBase):
    ...
    def __init__(self, source: str,
        ...
        super().__init__('minio', source, name, auth)

        is_secure=False
        if self.source.startswith('https://'):
            is_secure=True
            endpoint=self.source.replace('https://','')
        else:
            endpoint=self.source.replace('http://','')
        self.client = Minio(    endpoint=endpoint,
                                access_key=self.auth['username'],
                                secret_key=self.auth['password'],
                                secure=is_secure)
    
    def set_policy(self, bucket_policy: BucketPolicy = 'private'):
        LOGGER.debug(f"Set bucket_policy={bucket_policy} on {self.name}")
        if bucket_policy == 'readonly':
            self.client.set_bucket_policy(self.name, json.dumps(readonly_policy(self.name)))
        elif bucket_policy == 'readwrite':
            self.client.set_bucket_policy(self.name, json.dumps(readwrite_policy(self.name)))
        elif bucket_policy == 'private':
            self.client.delete_bucket_policy(self.name)
        else:
            LOGGER.warning(f"bucket_policy={bucket_policy} unknown")
    
    def create_bucket(self, bucket_policy: BucketPolicy = 'private'):
    ...
    def get(self, identifier: str) -> Any:
        ...
        try:
            response = self.client.get_object(self.name, object_name=identifier)
            data = response.data
    ...
    def put(self, filepath: Path, identifier: str) -> bool:
        ...
        self.client.fput_object(bucket_name=self.name, object_name=identifier, file_path=filepath)
    ...
    def put_bytes(self, data: bytes, identifier: str) -> bool:
        ...
        self.client.put_object(bucket_name=self.name, object_name=identifier, data=data, length=-1, part_size=10*1024*1024)
